...
class Timestamp(models.Model):
    ...
    updated_at = models.DateTimeField("更新时间", null=True, editable=False,
                                      auto_now=True)  # 调用 save() 会自动更新, 调用 objects.update 不会更新
...
class DynamicModel(object):
    _instance = dict()
    _tables = [table_info.name for table_info in connection.introspection.get_table_list(connection.cursor())]
    ...
    def __new__(cls, base_cls, db_table_suffix):
        ...
        if new_cls_name not in cls._instance:
            new_db_table = "{}_{}".format(base_cls._meta.db_table, db_table_suffix)  # noqa
            model_cls = type(new_cls_name, (base_cls,), {'__module__': base_cls.__module__})
            model_cls._meta.db_table = new_db_table  # noqa
